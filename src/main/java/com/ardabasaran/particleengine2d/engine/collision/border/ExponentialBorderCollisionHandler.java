...
public class ExponentialBorderCollisionHandler implements BorderCollisionHandler {
    ...
    public ExponentialBorderCollisionHandler(
            int universeWidth, int universeHeight, double timeDelta, double coefficient_of_restitution) {
    ...
    private void handleBorderCollision(
            Particle particle, double x, boolean scaleDown, Vector2D direction) {
        int POWER = 15;
        double FORCE_MIN = 0;
    ...
    public void resolveCollision(Particle particle) {
        ...
        if (posY + radius >= this.universeHeight) {
            handleBorderCollision(
                    particle, (this.universeHeight - posY) / radius, velY <= 0, new Vector2D(0, -1));
        }
        if (posX - radius <= 0) {
        ...
        if (posX + radius >= this.universeWidth) {
            handleBorderCollision(
                    particle, (this.universeWidth - posX) / radius, velX <= 0, new Vector2D(-1, 0));
        }
    }
}
