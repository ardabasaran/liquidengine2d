...
public class SortedCollisionDetector implements CollisionDetector {
    ...
    private int[][] calculateIntervals(List<Particle> sortedParticles) {
        ...
        for (int i = 0; i < numRanges; i++) {
            intervals[i][0] = -1;
            intervals[i][1] = -1;
            while (curr < sortedParticles.size()
                    && sortedParticles.get(curr).getPosition().getY() < i * minDiameter) {
                curr += 1;
            }
            ...
            intervals[i][0] = curr;
            while (curr < sortedParticles.size()
                    && sortedParticles.get(curr).getPosition().getY() < (i + 1) * minDiameter) {
                curr += 1;
            }
    ...
    private List<ParticlePair> resolveHorizontalCollisions(
        ...
        while (j < sortedParticles.size()
                && j <= intervals[currentVertical][1]
    ...
    private List<ParticlePair> resolveVerticalCollisions(
            List<Particle> sortedParticles,
            Particle particle,
            int[][] intervals,
            int currentVertical,
        ...
        while (j <= numVerticals) {
            if (currentVertical + j < numRanges) {
                ...
                int left =
                        Utilities.leftmostParticleIndex(
                                sortedParticles,
                                intervals[currentVertical + j][0],
                                intervals[currentVertical + j][1],
                                xRangeLow);
                int right =
                        Utilities.rightmostParticleIndex(
                                sortedParticles,
                                intervals[currentVertical + j][0],
                                intervals[currentVertical + j][1],
                                xRangeHigh);
    ...
    public Set<ParticlePair> detectCollisions(List<Particle> sortedParticles) {
        ...
        for (int i = 0; i < sortedParticles.size(); i++) {
            ...
            // Horizontal
            collisions.addAll(
                    resolveHorizontalCollisions(sortedParticles, particle, intervals, currentVertical, i));

            // Vertical
            int numVerticals = (int) Math.ceil(2 * particle.getRadius() / minDiameter);
            collisions.addAll(
                    resolveVerticalCollisions(
                            sortedParticles, particle, intervals, currentVertical, numVerticals));
        }
