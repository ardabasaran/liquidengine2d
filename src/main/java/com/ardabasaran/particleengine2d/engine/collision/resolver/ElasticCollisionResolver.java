...
public class ElasticCollisionResolver implements CollisionResolver {
  ...
  public void resolveCollision(Particle firstParticle, Particle secondParticle) {
    ...
    if (firstParticle.isMovingTowards(secondParticle)) {
      Vector2D firstParticleVelocityNormal = normal.scalarProduct(normal.dot(firstParticleVelocity));
      Vector2D secondParticleVelocityNormal = normal.scalarProduct(normal.dot(secondParticleVelocity));

      Vector2D firstParticleVelocityOrthogonal = orthogonal.scalarProduct(orthogonal.dot(firstParticleVelocity));
      Vector2D secondParticleVelocityOrthogonal = orthogonal.scalarProduct(orthogonal.dot(secondParticleVelocity));

      double totalMass = firstParticle.getMass() + secondParticle.getMass();
      Vector2D firstParticleVelocityNormalNew = (
          firstParticleVelocityNormal.scalarProduct(firstParticle.getMass() - secondParticle.getMass()).plus(
              secondParticleVelocityNormal.scalarProduct(2 * secondParticle.getMass()))).scalarProduct(1/totalMass);

      Vector2D secondParticleVelocityNormalNew = (
          secondParticleVelocityNormal.scalarProduct(secondParticle.getMass() - firstParticle.getMass()).plus(
              firstParticleVelocityNormal.scalarProduct(2 * firstParticle.getMass()))).scalarProduct(1/totalMass);
      ...
      firstParticle.setVelocity(firstParticleVelocityOrthogonal.plus(firstParticleVelocityNormalNew));
      secondParticle.setVelocity(secondParticleVelocityOrthogonal.plus(secondParticleVelocityNormalNew));
    }
  }
}
